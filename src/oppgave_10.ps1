...
$kortstokk = $kortstokk[2..$kortstokk.Count]                   #Kortstokken minus de to første kortene
...
while ((kortsum -kortstokk $magnus) -le (kortsum -kortstokk $meg)) {
    $magnus += $kortstokk[0]
    $kortstokk = $kortstokk[1..$kortstokk.Count]
}

### Magnus taper spillet dersom poengsummen er høyere enn 21
if ((kortsum -kortstokk $magnus) -gt $blackjack) {
    skrivUtResultat -vinner "meg" -kortStokkMagnus $magnus -kortStokkMeg $meg
    exit
}
